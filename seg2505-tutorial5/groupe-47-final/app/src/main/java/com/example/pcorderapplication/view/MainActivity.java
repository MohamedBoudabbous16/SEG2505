...
public class MainActivity extends AppCompatActivity {
    ...
    private final Map<String, String> localUserDatabase = new HashMap<>();
    ...
    private void initializeLocalUserDatabase() {
        localUserDatabase.put("assembler@gmail.com", "assembler123");
        localUserDatabase.put("storekeeper@gmail.com", "store123");
        localUserDatabase.put("administrator@gmail.com", "admin123");
    }

    private void authenticateUser(String email, String password) {
        if (email.isEmpty() || password.isEmpty()) {
            Toast.makeText(this, "Email and password are required!", Toast.LENGTH_SHORT).show();
            Log.w("Authentication", "Email and password are required!");
            return;
        }

        if (localUserDatabase.containsKey(email)) {
            if (localUserDatabase.get(email).equals(password)) {
                Log.d(TAG, "Local login successful for email: " + email);
                navigateToActivity(email);
            } else {
                Log.e(TAG, "Invalid credentials for local user: " + email);
                showErrorMessage();
            }
        } else {

            authenticateFirebaseUser(email, password);
        }
    }

    private void authenticateFirebaseUser(String email, String password) {
        mAuth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        FirebaseUser user = mAuth.getCurrentUser();
                        if (user != null) {
                            Log.d(TAG, "Firebase login successful for user: " + user.getEmail());
                            navigateToActivity("requester@example.com");
                        } else {
                            Log.e(TAG, "Firebase login failed: user is null after authentication");
                            showErrorMessage();
                        }
                    } else {
    ...
    private void navigateToActivity(String email) {
        Intent intent = null;
        ...
        switch (email) {
            ...
            case "assembler@gmail.com":
                intent = new Intent(this, AssemblerActivity.class);
                break;

            case "storekeeper@gmail.com":
        ...
        Log.d(TAG, "Navigating to activity for email: " + email);
        startActivity(intent);
    }
...
/*version stable de la mainActivity:
package com.example.pcorderapplication.view;
