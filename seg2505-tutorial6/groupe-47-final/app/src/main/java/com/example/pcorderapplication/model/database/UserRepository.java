...
public class UserRepository {
    ...
    // Méthode principales
    //
    // pour rechercher un utilisateur par email et rôle
    ...
    // pour rechercher un utilisateur par ID et rôle
    ...
    public Requester findRequesterByEmail(String email) {
    ...
    public Requester findRequesterById(int userId) {
    ...
    */
    ...
    // Méthodes génériques;
    //
    // pour trouver un utilisateur par email et rôle
    ...
    // pour trouver un utilisateur par ID et rôle
    ...
    // methode ajoutees pour finaliser le role d'administrator (ajouter un requester a la liste)
    public void addRequester(Requester requester) {
        UserInfo userInfo = new UserInfo(
                ...
                requester.getEmail(),
                requester.getPassword(),
                "Requester"
        );
        insertUser(userInfo);
    }
    public void deleteRequester(String email) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        try {
            ...
            if (rowsDeleted > 0) {
                Log.i("UserRepository", "Requester with email " + email + " deleted successfully.");
            } else {
                Log.i("UserRepository", "No requester found with email " + email + ".");
            }
        } catch (Exception e) {
            Log.e("UserRepository", "Error deleting requester: " + e.getMessage(), e);
        } finally {
            db.close();
        }
    }
    public int countRequesters() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        String query = "SELECT COUNT(*) FROM " + DatabaseSQLite.TABLE_USERS + " WHERE " + DatabaseSQLite.USER_ROLE + " = ?";
        ...
        cursor.close();
        db.close();
        Log.i("UserRepository", "Total Requesters: " + count);
        return count;
    }
    ...
    public List<Requester> getAllRequesters() {
        ...
        String selection = DatabaseSQLite.USER_ROLE + " = ?";
        String[] selectionArgs = {"Requester"};
        ...
        while (cursor.moveToNext()) {
            ...
            requesters.add(new Requester(firstName, lastName, email, password));
        }
